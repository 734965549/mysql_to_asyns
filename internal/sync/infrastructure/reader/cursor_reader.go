...
func NewCursorReader(db *sql.DB, schema, table string, identity *entity.TableIdentity) *CursorReader { // 创建流式读取器实例
...
func NewRangeShardingReader(db *sql.DB, schema, table string, identity *entity.TableIdentity) *RangeShardingReader { // 创建分片读取器实例
	pkColumn := "" // 初始化主键列
...
func NewReader(db *sql.DB, schema, table string, identity *entity.TableIdentity) DataReader { // 根据表标识创建读取器
