...
- **启动**: 启动任务（仅待执行或已暂停状态）；全量进行中暂停后再启动会从断点续传
- **暂停**: 暂停正在执行的任务；保留 `sync_phase` 与全量续传断点
...
### Q8b: 全量同步暂停后，再点启动会从头开始吗？

**默认不会**（未开启「DDL 前 DROP TABLE」时）：

- 已完整同步的表会被跳过
- 正在同步的表从上次**已提交**的主键位置继续
- 数值主键大表仍保持多线程并行分片续传

**例外**：

- 开启了「DDL 前 DROP TABLE」→ 每次启动重建目标表，必须从头全量
- 无主键表 / 非数值主键并行采样表 → 仅表级续传，未完成表整表重跑（`INSERT IGNORE` 幂等，不丢数据）

详情见 [全量续传指南](FULL_SYNC_RESUME_GUIDE.md)。
