...
func main() { // main函数是程序的入口点
	...
	cfg, err := config.LoadConfig("etc/application.toml") // 从指定路径加载配置文件
	if err != nil {                                       // 如果加载配置文件失败
		log.Printf("Warning: Failed to load config file: %v, using empty config", err) // 输出警告日志
		...
		if envErr := config.ApplyEnvOverrides(cfg); envErr != nil {
			log.Fatalf("Failed to apply environment overrides: %v", envErr)
		}
	...
	if cfg.Datasource.Host != "" {        // 检查是否配置了数据源主机
		log.Printf("Connecting to datasource database %s:%d/%s...", cfg.Datasource.Host, cfg.Datasource.Port, cfg.Datasource.Database) // 输出连接信息
		dsn := cfg.Datasource.GetDSN()                                                                                                 // 获取数据源名称
		db, err := sql.Open("mysql", dsn)                                                                                              // 打开数据库连接
		if err != nil {                                                                                                                // 如果连接失败
			log.Printf("Warning: Failed to connect to datasource database: %v", err) // 输出警告日志
		} else { // 如果连接成功
			// 测试连接
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second) // 创建带超时的上下文
			if err := db.PingContext(ctx); err != nil {                             // 测试数据库连接
				log.Printf("Warning: Failed to ping datasource database: %v", err) // 输出警告日志
			} else { // 如果连接测试成功
				log.Printf("Successfully connected to datasource database")           // 输出成功日志
		...
		log.Printf("No datasource configuration found in config file, metadata browser will use task-specific database connections") // 输出提示信息
	}
	...
	go func() { // 在新的goroutine中启动HTTP服务
		log.Printf("Server starting on %s", srv.Addr)                       // 输出启动信息
		log.Println("API Endpoints:")                                       // 输出API端点提示
		log.Println("  POST   /api/tasks           - Create task")          // 输出创建任务端点
		log.Println("  GET    /api/tasks           - List all tasks")       // 输出列出任务端点
		log.Println("  GET    /api/tasks/:id       - Get task details")     // 输出获取任务详情端点
		log.Println("  POST   /api/tasks/:id/start - Start task")           // 输出启动任务端点
		log.Println("  POST   /api/tasks/:id/pause - Pause task")           // 输出暂停任务端点
		log.Println("  GET    /api/tasks/:id/metrics - Get task metrics")   // 输出获取任务指标端点
		log.Println("  POST   /api/tasks/:id/skip  - Skip error")           // 输出跳过错误端点
		log.Println("  GET    /api/metadata/tables - List tables")          // 输出列出表端点
		log.Println("  GET    /api/metadata/identity - Get table identity") // 输出获取表标识端点
		log.Println("  GET    /api/health          - Health check")         // 输出健康检查端点

		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { // 启动HTTP服务器
			log.Fatalf("Failed to start server: %v", err) // 如果启动失败则输出错误并退出
		}
	}()
	...
	<-quit                                               // 阻塞等待信号
	log.Println("Shutting down server...")               // 输出关闭信息
	...
	if err := srv.Shutdown(ctx); err != nil { // 关闭HTTP服务器
		log.Printf("Server forced to shutdown: %v", err) // 如果关闭失败则输出错误
	}

	// 2. 关闭任务服务（保存所有任务状态并关闭数据库连接）
	if err := taskSvc.Close(); err != nil { // 关闭任务服务
		log.Printf("Error closing task service: %v", err) // 如果关闭失败则输出错误
	}

	log.Println("Server exited") // 输出退出信息
}
